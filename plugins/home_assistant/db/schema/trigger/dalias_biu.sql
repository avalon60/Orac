...
--changeset cbostock:home_assistant_trigger_dalias_biu context:plugin,prod labels:plugin,home_assistant stripComments:false splitStatements:false endDelimiter:/ runOnChange:true
create or replace trigger orac_ha.dalias_biu
before insert or update on orac_ha.device_aliases
for each row
declare
...
--rollback drop trigger orac_ha.dalias_biu;
