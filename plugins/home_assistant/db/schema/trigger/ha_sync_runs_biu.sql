...
--changeset cbostock:home_assistant_trigger_ha_sync_runs_biu context:plugin,prod labels:plugin,home_assistant stripComments:false splitStatements:false endDelimiter:/ runOnChange:true
create or replace trigger orac_ha.ha_sync_runs_biu
before insert or update on orac_ha.ha_sync_runs
for each row
declare
...
--rollback drop trigger orac_ha.ha_sync_runs_biu;
