...
--changeset clive:drop_box_trigger_drp_prf_biu context:plugin,prod labels:plugin,drop_box stripComments:false splitStatements:false endDelimiter:/ runOnChange:true
create or replace trigger orac_dropbox.drp_prf_biu
before insert or update on orac_dropbox.drop_processing_profile
for each row
declare
...
--rollback drop trigger orac_dropbox.drp_prf_biu;
